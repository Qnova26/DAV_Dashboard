...
def load_data():
    ...
    # Bersihkan nama kategori & subkategori dari inkonsistensi huruf besar/kecil
    # dan spasi ekstra, supaya "ATASAN", "atasan", "Atasan" dianggap satu nilai.
    ...
    # Parsing tanggal yang lebih fleksibel: coba format DD/MM/YYYY dahulu,
    # lalu untuk baris yang masih gagal (NaT), coba parser otomatis pandas.
    # Ini mengatasi kasus format tanggal yang tidak seragam di dalam CSV
    # gabungan (misalnya sebagian baris pakai format berbeda antar tahun).
...
with col4:
    ...
    st.plotly_chart(style_bar(fig4, 0.5), use_container_width=True, theme=None)
    chart_card_close()
...
with col9:
    ...
    st.plotly_chart(style_bar(fig9, 0.6), use_container_width=True, theme=None)
    chart_card_close()
...
with col10:
    ...
    st.plotly_chart(style_bar(fig10, 0.6), use_container_width=True, theme=None)
    chart_card_close()